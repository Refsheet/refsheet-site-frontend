...
import {useScrollPosition} from "@refsheet/hooks/useScrollPosition";
...
const PageHeader: React.FC<React.PropsWithChildren<IPageHeaderProps>> = ({
    ...
    const calculatePosition = () => {
        if (sectionRef.current && imageRef.current) {
            ...
            const scrollPercent = ((position + viewHeight) - sectionTop) / (viewHeight + sectionHeight);

            return Math.max(Math.min(scrollDistance * scrollPercent, scrollDistance), 0);
        } else {
            return 0;
        }
    }
