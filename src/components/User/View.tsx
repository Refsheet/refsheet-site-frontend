    ...
    <Main title={[user.name, "Users"]}>
      ...
      <Section container className="margin-top--large padding-bottom--none">
        <div className="sidebar-container">
          <div className="sidebar">
            <ul className="character-group-list margin-bottom--none">
              <li className="all fixed">
                ...
                <Link href={`/${user.username}`} className="name">
                  All Characters
                </Link>
                <span className="count">{NumberUtils.format(999999)}</span>
              </li>

              {user.character_groups?.map(
                (group) =>
                  group && (
                    <li className="something">
                      <i className="material-icons left folder">folder</i>
                      <Link href={`/${user.username}#${group.name}`}>
                        {group.name}
                      </Link>
                      <span className="count">
                        {NumberUtils.format(group.characters_count)}
                      </span>
                    </li>
                  ),
              )}
          ...
          <div className="main-content">( characters )</div>
        </div>
      </Section>
    </Main>
  );
